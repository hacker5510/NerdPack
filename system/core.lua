...
function gbl.Core.Round(_, num, idp)
	local mult = 10^(idp or 0)
	return math.floor(num * mult + 0.5) / mult
end

function gbl.Core.GetSpellID(_, spell)
...
local Run_Cache = {}
function gbl.Core.WhenInGame(_, func, prio)
	if Run_Cache then
...
function gbl.Core.HexToRGB(_, hex)
	return tonumber("0x"..hex:sub(1,2)), tonumber("0x"..hex:sub(3,4)), tonumber("0x"..hex:sub(5,6))
end

function gbl.Core.string_split(_, string, delimiter)
	local result, from = {}, 1
